...
export const HubHeader = React.memo(function HubHeader({ team, theme, position, record, currentWeek, weekLabel }: HubHeaderProps) {
  return (
    <View style={[styles.header, { borderBottomColor: `${theme.primary}60` }]}>
      <View style={styles.headerTop}>
        ...
        <View style={styles.titleWrap}>
          <Text style={[styles.teamName, { color: theme.primary !== Colors.white ? theme.primary : Colors.text }]}>
            {team.name}
          </Text>
...
const styles = StyleSheet.create({
  ...
  statChipRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.bg, padding: 10 },
  statChip: { flex: 1, alignItems: 'center' },
  statChipVal: { fontSize: 14, fontWeight: '900', color: Colors.text },
  statChipLabel: { fontSize: 9, color: Colors.textDim, fontWeight: '700', textTransform: 'uppercase', marginTop: 2 },
  statChipDivider: { width: 1, height: 28, backgroundColor: Colors.bgCard },
});
